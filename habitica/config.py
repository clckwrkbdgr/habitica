...
def load_auth(configfile=AUTH_CONF): # pragma: no cover
	...
	logger.debug('Loading habitica auth data from %s' % configfile)
	...
	try:
		...
		logger.error("Unable to find '%s'." % configfile)
		exit(1)
	...
	try:
		...
		logger.error("No 'Habitica' section in '%s'" % configfile)
		exit(1)

	except configparser.NoOptionError as e:
		logger.error("Missing option in auth file '%s': %s"
...
class Cache(object): # pragma: no cover -- TODO is it used after migration at all?
	...
	def load(self):
		logger.debug('Loading cached config data (%s)...' % self.configfile)
	...
	def update_quest_cache(self, **kwargs):
		logger.debug('Updating (and caching) config data (%s)...' % self.configfile)
